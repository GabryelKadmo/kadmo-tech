...
export default function AdminPage() {
    ...
    return (
        <div className="min-h-screen bg-black">
            <Header />
            <div className="px-2 py-10 flex flex-col items-center">
                        ...
                        <form className="flex flex-col gap-4" onSubmit={handleFormSubmit}>
                            ...
                            <div className="flex flex-col sm:flex-row items-center gap-4 mt-2 bg-gray-800/60 rounded-lg px-4 py-3 border border-gray-700">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    >
                                        <SelectTrigger className="w-32 bg-gray-800/80 border border-gray-700 text-white rounded-lg px-4 py-2 shadow focus:border-white/50 focus:ring-2 focus:ring-white/30">
                            ...
                            <DialogFooter className="flex flex-row gap-2 justify-end mt-2">
                                ...
                                <Button type="submit" disabled={formLoading} className="bg-white text-black font-semibold px-6 py-2 rounded-lg shadow-lg hover:bg-gray-100 hover:scale-105 transition-all">
                ...
                <div className="w-full max-w-7xl">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-10 gap-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight drop-shadow-lg">Painel <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-300">Admin</span></h1>
                                <p className="text-gray-300 text-sm md:text-base mt-1">Gerencie seus projetos de forma prática e visual.</p>
                            </div>
                        </div>
                        <Button type="button" size="lg" className="bg-white text-black font-semibold shadow-lg px-8 py-3 rounded-xl hover:bg-gray-100 hover:scale-105 transition-all" onClick={openAddModal}>
                            Adicionar Projeto
                        </Button>
                    </div>
                    {selectedProject && (
                        <div className="mb-10">
                            <Card className="bg-gradient-to-br py-[0px] from-gray-900/90 to-gray-800/80 border border-white/20 shadow-2xl">
                                <CardHeader>
                                    <CardTitle className="text-white text-xl flex items-center gap-2">
                                        <span className="text-gray-300 font-bold">#{selectedProject.id}</span> {selectedProject.title}
                                    </CardTitle>
                                </CardHeader>
                                <CardContent>
                                    ...
                                    {selectedProject.link && (
                                        <a href={selectedProject.link} target="_blank" rel="noopener noreferrer" className="text-gray-300 underline font-medium">Acessar Projeto</a>
                                    )}
                                </CardContent>
                            </Card>
                        </div>
                    ...
                    <div className="">
                        <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
                            <svg className="w-6 h-6 text-gray-300" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V7" /><path strokeLinecap="round" strokeLinejoin="round" d="M16 3v4M8 3v4M4 11h16" /></svg>
                            Projetos Ativos
                        </h2>
                        <Separator className="mb-6 bg-gradient-to-r from-white/40 to-gray-400/40 h-1 opacity-40" />
                        {loading && <div className="text-gray-400">Carregando...</div>}
                        {error && <div className="text-red-400 mb-2">{error}</div>}
                        {!loading && !error && (
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
                                {projects.map((project: projectApi.Project) => (
                                    <Card key={project.id} className="flex flex-col justify-between h-[410px] min-h-[410px] relative bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700 shadow-xl hover:shadow-white/10 transition-shadow group overflow-hidden rounded-2xl pb-8 pt-0">
                                        <div className="relative w-full mb-3 rounded-t-2xl overflow-hidden">
                                            {project.image ? (
                                                <div className="relative w-full h-48 overflow-hidden rounded-t-none">
                                                    {/* Blur background */}
                                                    <Image
                                                        src={project.image}
                                                        alt={project.title}
                                                ...
                                                <div className="relative w-full h-48 overflow-hidden rounded-t-2xl flex items-center justify-center bg-gray-800 text-gray-400">
                                            ...
                                            <div className="absolute right-4 top-4 z-10 flex gap-2">
                                                <Button
                                                    size="icon"
                                                    variant="secondary"
                                                    className="p-2 bg-white/90 border border-gray-300 hover:bg-white hover:border-gray-400 transition-colors shadow-md text-black"
                                                ...
                                                <div>
                                                    <div className="flex flex-nowrap gap-2 mb-2 overflow-x-auto scrollbar-none">
                                                        {project.tags?.map((tag: string, i: number) => (
                                                            <Badge key={i} className="bg-gray-700 text-white px-2 py-1 rounded-full text-xs font-semibold shadow">{tag}</Badge>
                                                        ))}
                                                    </div>
                                                    <p className="text-gray-300 mb-2 text-sm line-clamp-3">{project.description}</p>
                                                </div>
                                                {project.link && (
                                                    <a href={project.link} target="_blank" rel="noopener noreferrer" className="text-gray-300 underline font-medium mt-auto">Acessar Projeto</a>
                                                )}
                                            </CardContent>
                                        </div>
                                    </Card>
